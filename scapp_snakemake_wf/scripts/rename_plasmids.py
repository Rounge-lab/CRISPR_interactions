...
parser.add_argument('-n', '--samname', help='samplename', required=True)
...
def rename_sequences(infile, outfile, seqname):
    ...
    for i, record in enumerate(records):
        ...
        new_ids.append(samname+'_'+seqname+'_'+str(i+1)+'_plasmid_scapp')
        record.id = f"{samname}_{seqname}_{i+1}_plasmid_scapp"
        record.description = f"{samname}_{seqname}_{i+1}_plasmid_scapp"
